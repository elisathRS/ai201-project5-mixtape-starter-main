...
I used AI tools during investigation as a support tool for understanding code I had already narrowed down, not as a substitute for reading the implementation myself. After I identified a suspicious function, I asked the AI to explain edge cases, compare similar code paths, or clarify Python behavior relevant to the bug. For example, I used it to reason about the streak logic in [services/streak_service.py](services/streak_service.py) by asking what edge cases could cause the function to return the wrong value, and I also asked about the difference between Python’s `weekday()` and `isoweekday()` when I was checking the date logic. I also used it to compare the playlist and search service logic when I wanted a structural explanation of how those code paths differed.

This worked best when I had already found the suspicious code and wanted help interpreting it. The AI was useful for explaining what the code was doing and pointing out possible failure modes, but it was not reliable enough to diagnose the bug on its own without full context. In the streak case, the AI helped me frame the problem around the date comparison, but I still verified the diagnosis by reading the implementation directly and running the tests myself. In the search case, the AI gave a plausible high-level explanation, but I had to inspect the query in [services/search_service.py](services/search_service.py) directly to confirm the actual cause before changing anything. That workflow—find the suspicious code, use AI to help interpret it, then verify the diagnosis myself—was the approach I used throughout the project.
